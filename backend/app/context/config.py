...
@dataclass(slots=True, frozen=True)
class ContextConfig:
    database_path: Path = Path(".local/alim-context.db")
    debug: bool = False
    ...
    def from_env(cls) -> ContextConfig:
        budget = ContextBudgetConfig(
            max_context_tokens=_env_int("ALIM_MAX_CONTEXT_TOKENS", 65_536),
            reserve_output_tokens=_env_int("ALIM_RESERVED_OUTPUT_TOKENS", 16_384),
        ...
        embedding = EmbeddingConfig(
            base_url=os.getenv(
                ...
                os.getenv("ALIM_LLM_BASE_URL", "http://127.0.0.1:11434/v1"),
            ),
        ...
        return cls(
            database_path=Path(os.getenv("ALIM_CONTEXT_DB", ".local/alim-context.db")),
