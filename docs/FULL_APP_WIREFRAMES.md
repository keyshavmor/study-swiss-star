...
sequenceDiagram
  ...
  API->>Cloud: Verify user and thread ownership; save user message
  API->>PY: Current question and study context; stream=false
...
| --- | --- |
| Route IDs and screen ownership | `src/routes/**`, `docs/ROUTE_SCREEN_MAP.md` |
| Root providers and app shell | `src/routes/__root.tsx`, `src/components/app/AppShell.tsx` |
| Navigation | `src/components/app/AppHeader.tsx`, `src/components/app/MobileNavigation.tsx` |
| Local state and recurrence | `src/lib/store/app-data.tsx`, `src/lib/date-utils.ts` |
| Subject model and SPF mapping | `src/lib/mock/subjects.ts`, `docs/SUBJECT_MODEL_AND_LANGUAGE_RULES.md` |
| Swiss grade calculations | `src/lib/grade-math.ts`, `src/lib/mock/grades.ts` |
| Chat UI and transport | `src/components/StudyChat.tsx` |
| Cloud thread/message operations | `src/lib/chat.functions.ts`, `src/routes/api/chat.ts` |
| Python bridge | `src/lib/context-backend.server.ts` |
...
- Update a route by editing its file under `src/routes/`; never edit `src/routeTree.gen.ts`.
