...
## The `/api/chat` server route boundary (CURRENT — INTEGRATED)
...
- It forwards the verified bearer JWT, a matching `X-Student-Id`, and the effective seven-language response code. Request cancellation and a configured timeout abort the backend fetch.
- On backend failure it returns structured `{error:{code,message,retryable,request_id?}}`, preserves the status/request ID, and never includes the token. The chat UI shows a generic localized toast.
...
Status: the route and FastAPI implementation are current and covered by focused adapter/auth/language tests. Local model availability remains a deployment/runtime concern.
