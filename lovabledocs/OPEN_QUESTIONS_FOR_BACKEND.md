...
| --- | --- | --- | --- |
| 1 | Should chat thread/message persistence stay in Supabase or move to Python? | Two display writers would duplicate transcripts | **Resolved:** Supabase is the only durable transcript; Python reads it through the user-scoped adapter. |
...
| 8 | Should AI grading results be kept separate from the student's real grades? | Mixing practice with real grades corrupts averages | **Separate by default.** Durable grading history is in Supabase; promotion to an assessment remains explicit and `includeInStats` defaults to `false`. |
| 9 | Where do indexed materials live? | Determines whether uploads are indexable | **Resolved:** originals live in private Storage; chunks/embeddings live in user-scoped Postgres rows; computation remains local. |
...
| 12 | Does the backend need to identify the student? | Multi-student local use, per-student history | **Resolved:** send a Supabase bearer token, validate it, and derive ownership from verified `sub`; never trust `X-Student-Id`. |
