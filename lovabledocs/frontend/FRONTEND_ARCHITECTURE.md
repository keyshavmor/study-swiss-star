...
## The `/api/chat` server route boundary (CURRENT — FRONTEND / EXPECTED LOCAL BACKEND CONTRACT)
...
Status: the route itself is CURRENT — FRONTEND (fully implemented and shipped). The local Python backend's actual behaviour behind it is BACKEND GAP beyond the documented contract.
