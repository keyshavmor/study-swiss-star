...
| Settings | Switch — Auto storage cleanup | `PreferencesSections` | Toggle | `update({auto_storage_cleanup: checked})` | — | `user_preferences` — **CURRENT — SUPABASE** | consumption of the flag is **DEFERRED — NO CURRENT INTEGRATION**; the frontend's own `invokeEmergencyCleanup()` runs regardless of this flag whenever the RPC reports `emergency_cleanup_needed` | Supabase Postgres | as above | as above |
