...
- **Token attribution (fixed 2026-09-15)**: GitHub, LinkedIn and Spotify sign-ins also put a `provider_token` on the Supabase session, so a token is accepted as the Google Calendar token ONLY when a Google linking attempt is pending (`sessionStorage` marker `alim.google-calendar.connect-pending`, set immediately before `linkIdentity`) or the callback URL explicitly contains `google=connected`. Unattributed tokens from generic auth-state changes or a plain session refresh are ignored. The marker is cleared after a successful capture, on disconnect, and on a terminal connect failure.
